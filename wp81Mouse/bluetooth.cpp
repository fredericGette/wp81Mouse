...
void bleConnectionStop()
{
	...
	// Wait for the point where we request the "read" threads to stop.
...
void storeInitiatingDeviceInformation(BYTE* evtConCompltMsgReceived)
{
	...
	debug("Remote device address: %02X:%02X:%02X:%02X:%02X:%02X (type %s)\n", ia[0], ia[1], ia[2], ia[3], ia[4], ia[5], iat == 0 ? "public" : "random");
}
...
void storeRespondingDeviceInformation(BYTE* evtReadBdAddrCmdCompltMsgReceived)
{
	...
	debug("Local device address: %02X:%02X:%02X:%02X:%02X:%02X (type %s)\n", ra[0], ra[1], ra[2], ra[3], ra[4], ra[5], rat == 0 ? "public" : "random");
}
...
DWORD WINAPI readEvents(void* notUsed)
{
	...
	{
		debug("Failed to open wp81controldevice device! 0x%08X\n", GetLastError());
		return EXIT_FAILURE;
	}
	...
	debug("Start listening to Events...\n");
	while (readLoop_continue)
	{
		...
		if (success)
		{
			...
			{
				debug("Received: Command complete.\n");
			...
			{
				debug("Received: Read BD ADDR Command complete.\n");
			...
			{
				debug("Received: Connection complete.\n");
			...
			{
				//debug("Received: Number Of Completed Packets.\n");
		...
		{
			debug("Failed to send IOCTL_CONTROL_READ_HCI! 0x%X\n", GetLastError());
		}
	}
...
void parseAclData()
{
	...
	if (returned >= 12 && readAcl_outputBuffer[4] == 0x02) // ACL message
	{
		...
		case 0x04: // Attribute Protocol
			...
			{
				debug("Received: Exchange MTU Request.\n");
			...
			{
				debug("Received: Read By Type Request (2 bytes UUID).\n");
			}
			else if (isReadByGroupTypeRequestReceived())
			{
				debug("Received: Read By Group Type Request (2 bytes UUID).\n");
			}
			else if (isFindByTypeValueRequestReceived())
			{
				debug("Received: Find By Type Value Request.\n");
			}
			else if (isFindInformationRequestReceived())
			{
				debug("Received: Find Information Request.\n");
			}
			...
			{
				debug("Received: Read Blob Request.\n");
			}
			else if (isWriteRequestReceived())
			{
				debug("Received: Write Request.\n");
			}
			break;
		case 0x05: // Signaling Protocol
			...
			{
				debug("Received: Connection Parameter Update Response.\n");
			}
			break;
...
DWORD WINAPI readAclData(void* notUsed)
{
	...
	debug("Start listening to ACL Data...\n");
	while (readLoop_continue)
	{
...
int sendConnectionParameterUpdateRequest(HANDLE hciControlDeviceCmd, BYTE* cmd_inputBuffer, BYTE* cmd_outputBuffer)
{
	...
	debug("Sending Connection Parameter Update Request...\n");
	i = 0;
	...
	if (!success)
	{
		debug("Failed to send DeviceIoControl! 0x%08X\n", GetLastError());
		return 1;
	}
	else
	{
		debug("Connection Parameter Update Request sent.\n");
...
int pairing(HANDLE hciControlDeviceCmd, BYTE* cmd_inputBuffer, BYTE* cmd_outputBuffer, BYTE *ltkValue, ConnectionStatus *pConnectionStatus)
{
	BOOL success;
	DWORD returned;
	int i;

	*pConnectionStatus = PAIRING;

	debug("Sending Pairing Response...\n");
	i = 0;
	...
	debug("Pairing confirm computed.\n");

	debug("Sending Pairing Confirm...\n");
	i = 0;
	...
	{
		debug("Pairing Confirm sent.\n");
		printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');
	}


	debug("Waiting for the Pairing Random...\n");
	...
	debug("Waiting for the LTK Request...\n");
	...
	debug("Sending LTK Request Reply command...\n");
	i = 0;
	...
	{
		debug("LTK Request Reply sent.\n");
		printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');
	}

	debug("Waiting for the end of the LTK Request Reply command...\n");
	WaitForSingleObject(hEventCmdFinished, 1000);

	debug("Sending Encryption Information...\n");
	i = 0;
	cmd_inputBuffer[i++] = 0x19; // Length of the IOCTL message
	cmd_inputBuffer[i++] = 0x00;
	cmd_inputBuffer[i++] = 0x00;
	...
	{
		debug("Encryption Information sent.\n");
		printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');
	}


	debug("Sending Master Identification...\n");
	i = 0;
	...
	{
		debug("Master Identification sent.\n");
		printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');
	}

	debug("Waiting for the Signing Information...\n");
...
int bleConnectionStart(ConnectionStatus *pConnectionStatus, BYTE *pButton, int16_t *pX, int16_t *pY, int16_t *pWheel, int16_t *pHwheel, HANDLE hEventSendNotification)
{
	...
	{
		debug("Failed to send DeviceIoControl! 0x%08X\n", GetLastError());
		debug("Check Bluetooth is enabled.\n");
		return EXIT_FAILURE;
	}
...
reset:
	...
	debug("Waiting for the end of the Reset command...\n");
	WaitForSingleObject(hEventCmdFinished, 1000);

	debug("Sending Read BD_ADDR Command...\n");
	i = 0;
	cmd_inputBuffer[i++] = 0x03; // Length of the IOCTL message
	cmd_inputBuffer[i++] = 0x00;
	cmd_inputBuffer[i++] = 0x00;
	...
	{
		debug("Read BD_ADDR Command sent.\n");
		printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');
	}

	debug("Sending LE Set Advertising Parameters Command...\n");
	i = 0;
	...
	{
		debug("Set Advertising Parameters sent.\n");
		printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');
	}

	debug("Waiting for the end of the Set Advertising Parameters command...\n");
	WaitForSingleObject(hEventCmdFinished, 1000);

	debug("Sending LE Set Advertising Data Command...\n");
	i = 0;
	...
	{
		debug("Set Advertising Data sent.\n");
		printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');
	}

	debug("Waiting for the end of the Set Advertising Data command...\n");
	WaitForSingleObject(hEventCmdFinished, 1000);

	debug("Sending LE Set Advertise Enable Command...\n");
	i = 0;
	...
	{
		*pConnectionStatus = ADVERTISING;
		debug("Set Advertise Enable sent.\n");
		printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');
	}

	debug("Waiting for the end of the Set Advertise Enable command...\n");
	WaitForSingleObject(hEventCmdFinished, 1000);

	debug("Wait for the Pairing Request or for the LTK Request...\n");
	do
	{
		if (WaitForSingleObject(hEventAclDataReceived, 500) == WAIT_OBJECT_0)
		{
	...
	if (isPairingRequestReceived())
	{
		...
		if (pairing(hciControlDeviceCmd, cmd_inputBuffer, cmd_outputBuffer, ltkValue, pConnectionStatus) == EXIT_FAILURE) goto exit;
	}
	else
	{
		...
		|| connectionLatency[0] != 0x00 || connectionLatency[1] != 0x00)
	{
		...
		debug("Waiting for the Connection Parameter Update Response...\n");
		...
		debug("Waiting for the Connection Update Complete event...\n");
	...
	debug("Main loop...\n");
	*pConnectionStatus = SERVING_ATTRIBUTES;
	while (mainLoop_continue)
	{
		if (isExchangeMTURequestReceived())
		{
			debug("Sending Exchange MTU Response...\n");
			i = 0;
			...
			{
				debug("Exchange MTU Response sent.\n");
		...
		{
			debug("Sending Read By Type Response for GATT Characteristic Declaration...\n");
			i = 0;
			...
			{
				debug("Read By Type Response sent.\n");
				attMsgReceived[0] = 0x00; // We set the first byte to 0x00 to not process 2 times the same ATT message.
				printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');
			}
		}
		...
		{
			debug("Sending Read By Type Response for PnP ID...\n");
			i = 0;
			cmd_inputBuffer[i++] = 0x13; // Length of the IOCTL message
			cmd_inputBuffer[i++] = 0x00;
			cmd_inputBuffer[i++] = 0x00;
		...
		{
			debug("Sending Read By Type Response for Peripheral Preferred Connection Parameters...\n");
			i = 0;
			cmd_inputBuffer[i++] = 0x14; // Length of the IOCTL message
			cmd_inputBuffer[i++] = 0x00;
			cmd_inputBuffer[i++] = 0x00;
		...
		{
			debug("Sending Read By Group Type Response for GATT Primary Service Declaration...\n");
			i = 0;
			...
			{
				debug("Read By Group Type Response sent.\n");
				attMsgReceived[0] = 0x00; // We set the first byte to 0x00 to not process 2 times the same ATT message.
				printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');
			}
		}
			...
			, 0x08, 0x00, 0x08, 0x00))
		{
			debug("Sending Find By Type Value Response for type GATT Primary Service Declaration and value Generic Attribute Profile...\n");
			i = 0;
			...
			{
				debug("Find By Type Value Response sent.\n");
				attMsgReceived[0] = 0x00; // We set the first byte to 0x00 to not process 2 times the same ATT message.
				printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');
			}
		}
		...
		{
			debug("Sending Find Information Response for Report...\n");
			i = 0;
			...
			{
				debug("Find Information Response sent.\n");
				attMsgReceived[0] = 0x00; // We set the first byte to 0x00 to not process 2 times the same ATT message.
				printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');
			}
		}
		...
		{
			debug("Sending Read Response for Generic Access Profile: Device Name...\n");
			i = 0;
		...
		{
			debug("Sending Read Response for Appearance...\n");
			i = 0;
			cmd_inputBuffer[i++] = 0x0B; // Length of the IOCTL message
			cmd_inputBuffer[i++] = 0x00;
			cmd_inputBuffer[i++] = 0x00;
		...
		{
			debug("Sending Read Response for Peripheral Preferred Connection Parameters...\n");
			i = 0;
			cmd_inputBuffer[i++] = 0x11; // Length of the IOCTL message
			cmd_inputBuffer[i++] = 0x00;
			cmd_inputBuffer[i++] = 0x00;
		...
		{
			debug("Sending Read Response for Manufacturer Name String...\n");
			i = 0;
			cmd_inputBuffer[i++] = 0x12; // Length of the IOCTL message
			cmd_inputBuffer[i++] = 0x00;
			cmd_inputBuffer[i++] = 0x00;
		...
		{
			debug("Sending Read Response for Device Information: PnP ID...\n");
			i = 0;
		...
		{
			debug("Sending Read Response for Boot Mouse Input Report...\n");
			i = 0;
			cmd_inputBuffer[i++] = 0x11; // Length of the IOCTL message
			cmd_inputBuffer[i++] = 0x00;
			cmd_inputBuffer[i++] = 0x00;
		...
		{
			debug("Sending Read Response for Boot Mouse Input Report : Client Characteristic Configuration...\n");
			i = 0;
			cmd_inputBuffer[i++] = 0x0B; // Length of the IOCTL message
			cmd_inputBuffer[i++] = 0x00;
			cmd_inputBuffer[i++] = 0x00;
		...
		{
			debug("Sending Read Response for Human Interface Device: Protocol Mode...\n");
			i = 0;
		...
		{
			debug("Sending Read Response for Human Interface Device: Report...\n");
			i = 0;
		...
		{
			debug("Sending Read Response for Human Interface Device: Report: Client Characteristic Configuration...\n");
			i = 0;
			cmd_inputBuffer[i++] = 0x0B; // Length of the IOCTL message
			cmd_inputBuffer[i++] = 0x00;
			cmd_inputBuffer[i++] = 0x00;
		...
		{
			debug("Sending Read Response for Human Interface Device: Report: Report Reference...\n");
			i = 0;
			cmd_inputBuffer[i++] = 0x0B; // Length of the IOCTL message
			cmd_inputBuffer[i++] = 0x00;
			cmd_inputBuffer[i++] = 0x00;
		...
		{
			debug("Sending Read Response for Human Interface Device: Report Map...\n");
			i = 0;
		...
		{
			debug("Sending Read Blob Response for Human Interface Device: Report Map...\n");
			i = 0;
			cmd_inputBuffer[i++] = 0x1F; // Length of the IOCTL message
			cmd_inputBuffer[i++] = 0x00;
			cmd_inputBuffer[i++] = 0x00;
		...
		{
			debug("Sending Read Response for Human Interface Device: HID Information...\n");
			i = 0;
			cmd_inputBuffer[i++] = 0x0D; // Length of the IOCTL message
			cmd_inputBuffer[i++] = 0x00;
			cmd_inputBuffer[i++] = 0x00;
		...
		debug("Waiting for the new ACL data...\n");
	...
	*pConnectionStatus = SENDING_NOTIFICATIONS;
	...
	while (mainLoop_continue)
	{
		// Even when there's nothing to send, we have to send a notification from time to time 
		// otherwise we will reach the supervision timeout and the connection will be closed.
		WaitForSingleObject(hEventSendNotification, 1000);
		ResetEvent(hEventSendNotification);

		//debug("Sending Handle Value Notification...\n");
		i = 0;
		cmd_inputBuffer[i++] = 0x14; // Length of the IOCTL message
		cmd_inputBuffer[i++] = 0x00;
		cmd_inputBuffer[i++] = 0x00;
		...
		cmd_inputBuffer[i++] = 0x00; //
		cmd_inputBuffer[i++] = *pButton; // 5 Buttons 0b00011111
		cmd_inputBuffer[i++] = *pX & 0xFF; // lsb X
		cmd_inputBuffer[i++] = *pX >> 8; // msb X
		cmd_inputBuffer[i++] = *pY & 0xFF; // lsb Y
		cmd_inputBuffer[i++] = *pY >> 8; // msb Y
		cmd_inputBuffer[i++] = *pWheel & 0xFF; // lsb Wheel
		...
		{
			//debug("Handle Value Notification sent.\n");
			//printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');

			nbPacketNotCompleted++;
			if (nbPacketNotCompleted > 20)
			{
				printStringValue("Too many packets not completed!", nbPacketNotCompleted);
				// Too many packets are waiting to be sent.
				// There might be a problem, we reset the connection.
				goto reset;
			}
		}
	}
...
exit:
	...
	{
		debug("Send LE Set Advertise Disable Command\n");
		i = 0;
		cmd_inputBuffer[i++] = 0x04; // Length of the IOCTL message
		cmd_inputBuffer[i++] = 0x00;
		cmd_inputBuffer[i++] = 0x00;
		...
		if (!success)
		{
			debug("Failed to send DeviceIoControl! 0x%08X", GetLastError());
		}
		else
		{
			debug("Set Advertise Disable sent\n");
			printBuffer2HexString(cmd_outputBuffer, returned, TRUE, 'c');
			ResetEvent(hEventCmdFinished);
		}

		debug("Wait for the end of the Set Advertise Disable command\n");
		WaitForSingleObject(hEventCmdFinished, 1000);
	}


	readLoop_continue = FALSE;
	debug("Stop then start the Bluetooth of the phone to exit this program.\n");
